...
recognition challenge~\cite{ukbmdrmf,cao2025respiratoryburden,cao2026copd}.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/figure_input_encoding}
  \caption{Overview of the five experimental configurations and their encoding pipelines.
...
\paragraph{Input Representation (4 representation families).}
Patient data before age 60 is serialised into five distinct configurations
across four representation families, of increasing temporal and modality
richness:
...
\begin{enumerate}
  ...
  \item \textbf{Sequential ordinal (Setting 2).}
        ...
        \textit{``42.0yr:J45 $\rightarrow$ 52.1yr:F32 $\rightarrow$ \ldots''}.
  ...
  \item \textbf{Isolated temporal prose (Setting 3).}
        ...
        \textit{``At age 42.0, patient was diagnosed with J45 asthma.''}
...
We report four metrics on the held-out test set:
...
(iv) \emph{Bootstrap confidence intervals:} 1{,}000 paired resamples of the test set to compute 95\% CIs for C-index and paired difference $p$-values for key contrasts (S4 vs.\ S3, S4 vs.\ S1, S3-Full vs.\ S1).
All metrics are computed with \texttt{lifelines} and \texttt{scikit-survival}.
No information leakage: PCA projections and CoxPH coefficients are fitted
on the training split only; validation and test transforms use
