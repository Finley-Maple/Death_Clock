...
\begin{enumerate}
  ...
  \item \textbf{Sequential ordinal (Setting 2).}
        ...
        a 7B-parameter autoregressive LLM pre-trained on EHR sequences,
        which we evaluate in zero-shot mode.
  ...
  \item \textbf{Decoupled time and event (Setting 4).}
        ...
        streams: the age is mapped to a 128-dim sinusoidal encoding
        (matching Delphi's AgeEncoding), and each ICD token string is
...
\begin{itemize}
  ...
  \item \textbf{Bio\_ClinicalBERT}~\cite{bioclinicalbert}: a 110M-parameter
        ...
        causing truncation of the disease-history tail --- reported as a
        limitation.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Element-wise summation (sum)}: the embedding is
        ...
        compression.
        This tests whether combining modalities additively in a shared latent
        space improves over stacking them.
\end{itemize}
...
\caption{Experiment matrix. Settings 1 and 2 have no encoder or fusion axis.
         ...
         Cells marked $\star$ require GPU embedding runs on the RTX 3090.}
\label{tab:matrix}
\begin{tabular}{llcc}
\toprule
