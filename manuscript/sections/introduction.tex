...
first metabolic event and a subsequent cardiovascular diagnosis carries
prognostically relevant temporal structure that token proximity alone cannot
preserve.
...
patient record as an unordered feature set or as a structured temporal risk
trajectory.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/figure_0}
  \caption{Prediction task overview.
           (A) Three illustrative patient timelines from the UK Biobank
           respiratory cohort ($n$=124,158).
           Blue dots mark ICD-10 diagnosis events before the age-60
           feature-extraction cutoff; a red cross indicates death, an open
           circle indicates censoring.
           (B) Illustrative model output: a CoxPH survival curve
           $S(t\mid x)$ stratified by predicted risk group.
           Evaluation metrics (C-index, TD-AUC, IBS) measure discrimination
           and calibration of these curves on the held-out test set.}
  \label{fig:task}
\end{figure}
...
disease subgroup ($n\,=\,124{,}158$)~\cite{cao2025respiratoryburden,cao2026copd},
a population where mortality prediction is especially challenging due to
multimorbidity burden and heterogeneous temporal disease trajectories~\cite{ukbmdrmf}.
Our primary dimension varies \emph{input representation} across five
experimental configurations spanning four representation families, from
cross-sectional bag-of-features to unified clinical prose, all under a
uniform CoxPH survival head.
...
Absolute discrimination gains are modest; the scientific value of the design
lies in controlled isolation of which encoding choices produce statistically
consistent differences under a uniform survival head.

Our main contributions are:
\begin{itemize}
  \item A controlled benchmark of four temporal-encoding representation
        families (five experimental configurations) for LLM-based mortality
        prediction, the first to isolate input serialisation as the sole
        experimental variable under a uniform survival head.

  \item Evidence that explicit geometric time separation provides a small
        but statistically consistent gain: decoupled age--event sinusoidal
        encoding (Setting~4) achieves a $+$0.008 C-index advantage (bootstrap
        95\,\% CI: $+$0.005 to $+$0.010, $p<0.001$) over prose serialisation
        of identical events.

  \item Evidence that LLMs encode numeric biomarker structure from prose:
        full-text prose embedding (Setting~3-Full) with \emph{no raw features}
        matches a 39-feature CoxPH baseline, and that zero-shot autoregressive
        generation faces a calibration gap (Delphi IBS\,=\,0.185 vs.\ 0.141,
        AUC\,=\,0.52 at the earliest evaluated horizon) partly due to missing
        acute biomarker inputs.

  \item Ablation evidence that encoder choice and fusion method are
        secondary to serialisation format: among reference (concat)
        configurations, Bio\_ClinicalBERT and Qwen3-8B differ by at most
        0.007 C-index (less than 0.001 in Setting~4); concatenation
        outperforms summation by $+$0.028--$+$0.039.
\end{itemize}
