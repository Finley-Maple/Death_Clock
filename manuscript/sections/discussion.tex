...
Delphi's IBS of 0.185 vs.\ $\approx$0.141 for CoxPH variants partly reflects
a covariate asymmetry: Delphi has no access to HbA1c, CRP, creatinine, or
cystatin~C, so the gap captures missing acute biomarker signal as much as a
deficiency of the autoregressive paradigm.
The near-term AUC collapse (0.52 at 9.5\,yr) is consistent with this: short-
horizon mortality is dominated by physiological derangements absent from the
ICD token stream, an inherent constraint of zero-shot deployment under
distribution shift from pre-training to the UK Biobank cohort.
Fine-tuning with a calibration-aware objective and numeric biomarker inputs
would likely close this gap, but at the cost of zero-shot generalisability.
...
The primary evaluation metrics (C-index, TD-AUC) measure discrimination; IBS and
RMST ($\int_0^\tau S(t\mid x)\,\mathrm{d}t$) complement these with calibration
and patient-interpretable life-year estimates respectively.
