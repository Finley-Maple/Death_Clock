...
Our primary axis \textbf{(A)} varies \emph{input representation} across five
...
To verify that findings are not encoder- or fusion-specific, we include two
ablation axes: \textbf{(B)} \emph{encoder architecture} (Qwen3-8B vs.\
Bio\_ClinicalBERT) and \textbf{(C)} \emph{feature fusion method}
(concatenation vs.\ element-wise summation).
