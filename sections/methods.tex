...
\begin{figure}[ht]
  ...
  \caption{Overview of the five experimental settings and their encoding pipelines.
           \textbf{S1} (top): demographics, biomarkers, and comorbidities are
           concatenated into a fixed-dimension feature vector for CoxPH.
           \textbf{S2}: age-tagged ICD tokens are fed to a GPT-based generative
           transformer (Delphi) in zero-shot mode.
           \textbf{S3}: disease history is serialised as natural-language prose
           and encoded by a frozen LLM; the resulting embedding is fused with
           raw features.
           ...
           \textbf{S5} (bottom): all modalities are combined into a single paragraph
           and encoded by the LLM with no raw features concatenated.}
  \label{fig:overview}
\end{figure}
...
\paragraph{Encoder Architecture (Settings 3--5).}
...
2-fusion ablation for Settings 3--4 and 2-encoder ablation for Setting~5
yields \textbf{12 total cells}.
S1 and S2 serve as non-embedding baselines; the 10 embedding cells
(S3--S5, both encoders, both fusion methods where applicable)
